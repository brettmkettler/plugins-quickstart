...
def summarize_incident_comments(comments, work_notes, comments_and_work_notes, close_notes, incident_resolvedby):
    ...
    incident_prompt = (
        "How would you summarize the work notes and comments please provide a very detailed 2 paragraph solution with steps on how to fix the issue. You will answer the question each time like the issue is not already resolved and put the name of the person who resolved the issue at the end of the paragraph. \n"
        + "\n"
...
def analyze_incidents(description, incidents):
    ...
    for incident in incidents:
        ...
        if similarity_score.item() > similarity_threshold:
            ...
            summarize = summarize_incident_comments(incident_comments, incident_worknotes, incident_comments_and_worknotes, incident_closenotes, incident_resolvedby)
...
def generate_recommendation(username, password, description, work_notes, comments, relevantinfo, incident_number):
    ...
    incident_prompt = (
        "You are a helpful AI Assistant named Gemi. Your job is to help resolve this incident given the incident description, work notes, comments, and relevant incident details. Please provide a very detailed 2 paragraph recommending a step by step solution to this incident based on the information given below. In addition, provide the names of each person that has resolved similar incidents in the past, and the count of the number of incidents that each person has resolved. Also, recommend to the user that they can ask for a more detailed recommendation if needed: "
        + "\n"
        + "Incident Description: "
        + description
        + "\n"
...
def generate_recommendation_route():
    ...
    try:
        ...
        results = generate_recommendation(username, password, description, work_notes, comments, relevantinfo, incident_number)
